...
//        var cameraEntity = scene.EntityByName("UiCamera");
        ...
        if(!selectedObject.GetComponent("EC_Material"))
        {
...
function MoveSelected(/*QPoint(F)*/pos)
{
    ...
    if (selectedObject)
    {
        ...
        var newPosition;
        var attachedToCamera = selectedObject.dynamiccomponent.GetAttribute("AttachedToCamera");
        if(!attachedToCamera)
            newPosition = selectedPosition.Add(cameraEnt.placeable.Orientation().Mul(new float3(amountX, -amountY, 0)));
        else
            newPosition = selectedPosition.Add(cameraEnt.placeable.Orientation().Mul(new float3(-amountX, -amountY, 0)));
