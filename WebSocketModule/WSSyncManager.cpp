...
WSSyncManager::WSSyncManager(TundraLogicModule* owner, WebSocketManager* wsmanager) :
    owner_(owner),
    framework_(owner->GetFramework()),
    websocketmanager_(wsmanager),
...
WSSyncManager::~WSSyncManager()
{

}

void WSSyncManager::SetUpdatePeriod(float period)
{
    // Allow max 100fps
    if (period < 0.01f)
        period = 0.01f;
...
void WSSyncManager::Update(f64 frametime)
{
    ...
    for(UserConnectionList::iterator i = users.begin(); i != users.end(); ++i)
        if ((*i)->syncState)
        {
            ...
            ProcessSyncState((*i)->ConnectionId(), (*i)->syncState.get());
        }
...
void WSSyncManager::NewUserConnected(const UserConnectionPtr &user)
{
    ...
    if (!scene)
    {
        LogWarning("WSSyncManager: Cannot handle new user connection message - No scene set!");
        return;
    }
...
SceneSyncState* WSSyncManager::SceneState(int connectionId) const
{
    ...
    UserConnectionPtr connection;

    foreach(const UserConnectionPtr &user, connections)
        if (user->userID == connectionId)
            connection =  user;
    connection =  UserConnectionPtr();

    return SceneState(connection);
}

SceneSyncState* WSSyncManager::SceneState(const UserConnectionPtr &connection) const
{
    if (!owner_->IsServer())
        return 0;
    if (!connection)
...
void WSSyncManager::RegisterToScene(ScenePtr scene)
{
    ...
    if (!scene)
    {
        LogError("WSSyncManager::RegisterToScene: Null scene, cannot replicate");
        return;
    }
...
/*
void WSSyncManager::HandleKristalliMessage(kNet::MessageConnection* source, kNet::packet_id_t packetId, kNet::message_id_t messageId, const char* data, size_t numBytes)
{
    try
    {
        switch(messageId)
...
*/
void WSSyncManager::OnAttributeChanged(IComponent* comp, IAttribute* attr, AttributeChange::Type change)
{
    assert(comp && attr);
    if (!comp || !attr)
        return;
...
void WSSyncManager::OnAttributeAdded(IComponent* comp, IAttribute* attr, AttributeChange::Type change)
{
    assert(comp && attr);
    if (!comp || !attr)
        return;
...
void WSSyncManager::OnAttributeRemoved(IComponent* comp, IAttribute* attr, AttributeChange::Type change)
{
    assert(comp && attr);
    if (!comp || !attr)
        return;
...
void WSSyncManager::OnComponentAdded(Entity* entity, IComponent* comp, AttributeChange::Type change)
{
    assert(entity && comp);
    if (!entity || !comp)
        return;
...
void WSSyncManager::OnComponentRemoved(Entity* entity, IComponent* comp, AttributeChange::Type change)
{
    assert(entity && comp);
    if (!entity || !comp)
        return;
...
void WSSyncManager::OnEntityCreated(Entity* entity, AttributeChange::Type change)
{
...
void WSSyncManager::OnEntityRemoved(Entity* entity, AttributeChange::Type change)
{
    assert(entity);
    if (!entity)
        return;
...
void WSSyncManager::OnActionTriggered(Entity *entity, const QString &action, const QStringList &params, EntityAction::ExecTypeField type)
{
...
void WSSyncManager::OnUserActionTriggered(UserConnection* user, Entity *entity, const QString &action, const QStringList &params)
{
    assert(user && entity);
    if (!entity || !user)
        return;
...
void WSSyncManager::InterpolateRigidBodies(f64 frametime, SceneSyncState* state)
{
    ScenePtr scene = scene_.lock();
    if (!scene)
        return;
...
void WSSyncManager::ReplicateRigidBodyChanges(kNet::MessageConnection* destination, SceneSyncState* state)
{
...
void WSSyncManager::QueueMessage(u8 clientId, kNet::message_id_t id, bool reliable, bool inOrder, kNet::DataSerializer& ds)
{
    /*
    kNet::NetworkMessage* msg = connection->StartNewMessage(id, ds.BytesFilled());
    memcpy(data, ds.GetData(), ds.BytesFilled());
    ...
    char* msg;
    //memcpy(msg, ds.GetData(), ds.BytesFilled());
...
void WSSyncManager::SendMessage(u8 clientId, Json::Value msg, string event) {
    ...
    LogDebug(QString::fromStdString(json));

    websocketmanager_->sendJsonToClient(json, clientId);
}

void WSSyncManager::ProcessSyncState(u8 clientId, SceneSyncState* state)
{
    ...
    while (!state->dirtyQueue.empty())
    {
        ...
        else if (entityState.isNew)
        {
            ...
            for (Entity::ComponentMap::const_iterator i = components.begin(); i != components.end(); ++i)
            {
                ...
                WriteComponentFullUpdate(comps, comp);
...
bool WSSyncManager::ValidateAction(kNet::MessageConnection* source, unsigned messageID, entity_id_t entityID)
{
    assert(source);
...
/*
void WSSyncManager::HandleRigidBodyChanges(kNet::MessageConnection* source, kNet::packet_id_t packetId, const char* data, size_t numBytes)
{
    ScenePtr scene = scene_.lock();
    if (!scene)
        return;
...
void WSSyncManager::HandleCreateEntity(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    UserConnectionPtr user = owner_->GetKristalliModule()->GetUserConnection(source);
...
void WSSyncManager::HandleCreateComponents(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    // Get matching syncstate for reflecting the changes
    SceneSyncState* state = GetSceneSyncState(source);
...
void WSSyncManager::HandleRemoveEntity(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    // Get matching syncstate for reflecting the changes
    SceneSyncState* state = GetSceneSyncState(source);
...
void WSSyncManager::HandleRemoveComponents(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    // Get matching syncstate for reflecting the changes
    SceneSyncState* state = GetSceneSyncState(source);
...
void WSSyncManager::HandleCreateAttributes(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    // Get matching syncstate for reflecting the changes
    SceneSyncState* state = GetSceneSyncState(source);
...
void WSSyncManager::HandleRemoveAttributes(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    // Get matching syncstate for reflecting the changes
    SceneSyncState* state = GetSceneSyncState(source);
...
void WSSyncManager::HandleCreateComponentsReply(kNet::MessageConnection* source, const char* data, size_t numBytes)
{
    assert(source);
    SceneSyncState* state = GetSceneSyncState(source);
    ScenePtr scene = GetRegisteredScene();
...
void WSSyncManager::HandleEntityAction(kNet::MessageConnection* source, MsgEntityAction& msg)
{
    bool isServer = owner_->IsServer();
    ...
    if (!scene)
    {
        LogWarning("WSSyncManager: Ignoring received MsgEntityAction \"" + QString(msg.name.size() == 0 ? "(null)" : std::string((const char *)&msg.name[0], msg.name.size()).c_str()) + "\" (" + QString::number(msg.parameters.size()) + " parameters) for entity ID " + QString::number(msg.entityId) + " as no scene exists!");
        return;
    }
    ...
    if (isServer && (type & EntityAction::Peers) != 0)
    {
        ...
        foreach(UserConnectionPtr userConn, connections)
    ...
    if (!handled)
        LogWarning("WSSyncManager: Received MsgEntityAction message \"" + action + "\", but it went unhandled because of its type=" + QString::number(type));
